...
{
    int icnt, dev_cnt=get_usb_dev_cnt();

    if(g_uiDevNum==0)
    {
	for (icnt = 0; icnt < dev_cnt; icnt++)
	{
...
int FlashIdentifier(CHIP_INFO*Chip_Info, int search_all,int Index)
{
	...
	if (UniqueID != 0)
	{
	    rc = Dedi_Search_Chip_Db(0x9f, UniqueID, Chip_Info);
	    if(rc && (search_all == 0))
	    {
		if(c==1)
		    isSendFFsequence=true;
    ...
    if (UniqueID != 0)
    {
	rc = Dedi_Search_Chip_Db(0x15, UniqueID, Chip_Info);
	if(rc && (search_all == 0))
	    return rc;
    }
    UniqueID = 0;
    ...
    if (UniqueID != 0)
    {
	rc = Dedi_Search_Chip_Db(0xab, UniqueID, Chip_Info);
	if(rc && (search_all == 0))
	    return rc;
    }
    UniqueID = 0;
    ...
    if (UniqueID != 0)
    {
	rc = Dedi_Search_Chip_Db(0x90, UniqueID, Chip_Info);
	if(rc && (search_all == 0))
	    return rc;
    }
    UniqueID = 0;
